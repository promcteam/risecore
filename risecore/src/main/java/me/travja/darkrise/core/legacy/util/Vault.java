...
public final class Vault {
    ...
    public static void init() {
        ...
        vault = true;
        final RegisteredServiceProvider<Economy> economyProvider = Core.getInstance().getServer().getServicesManager().getRegistration(Economy.class);
        if (economyProvider != null) {
            eco = economyProvider.getProvider();
            Core.getInstance().getLogger().info("Hooked into vault for economy.");
        }
        final RegisteredServiceProvider<Permission> permissionProvider = Core.getInstance().getServer().getServicesManager().getRegistration(Permission.class);
